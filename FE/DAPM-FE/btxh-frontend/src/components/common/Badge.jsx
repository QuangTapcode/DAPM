const CONFIG = {
  ...
  'Đang xử lý': { label: 'Đang xử lý', cls: 'bg-sky-100 text-sky-800 border-sky-200' },
  ...
  'Chờ xác minh': { label: 'Chờ xác minh', cls: 'bg-amber-100 text-amber-800 border-amber-200' },
  'Hợp lệ': { label: 'Hợp lệ', cls: 'bg-green-100 text-green-800 border-green-200' },
  'Hết hạn': { label: 'Hết hạn', cls: 'bg-slate-100 text-slate-700 border-slate-200' },
...
export function Badge({ status, label, size = 'sm', className = '' }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 rounded-full border font-medium ${sizeClass} ${cfg.cls} ${className}`}>
