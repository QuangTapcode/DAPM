...
function normalizeRequest(req, requestId) {
  return {
    MaYeuCauNhan: req.MaYeuCauNhan || req.maYeuCauNhan || req.id || requestId,
    MaHoSoNhanNuoi: req.MaHoSoNhanNuoi || req.maHoSoNhanNuoi || '',
    TenNguoiNhan: req.TenNguoiNhan || req.tenNguoiNhan || req.adopterName || '',
    SDTNguoiNhan: req.SDTNguoiNhan || req.sDTNguoiNhan || req.sdtNguoiNhan || req.phone || '',
    NgaySinhNguoiNhan:
      req.NgaySinhNguoiNhan || req.ngaySinhNguoiNhan || req.birthDate || req.dateOfBirth || '',
    NgheNghiep: req.NgheNghiep || req.ngheNghiep || req.job || '',
    ThuNhapHangThang: req.ThuNhapHangThang ?? req.thuNhapHangThang ?? req.monthlyIncome,
    LyDoNhanNuoi: req.LyDoNhanNuoi || req.lyDoNhanNuoi || req.motivation || '',
    MongMuonVeTre: req.MongMuonVeTre || req.mongMuonVeTre || req.expectedChild || '',
    TrangThai: req.TrangThai || req.trangThai || req.status || 'Chờ ghép trẻ',
    SoGiayTo: req.SoGiayTo ?? req.soGiayTo ?? (Array.isArray(req.GiayTos || req.giayTos) ? (req.GiayTos || req.giayTos).length : 4),
    SoGiayToHopLe: req.SoGiayToHopLe ?? req.soGiayToHopLe ?? (Array.isArray(req.GiayTos || req.giayTos) ? (req.GiayTos || req.giayTos).length : 4),
    GiayTo: req.GiayTo || req.giayTo || req.documents || fallbackRequest.GiayTo,
  };
}

function normalizeChild(item) {
  return {
    MaTre: item.MaTre || item.maTre || item.id || '',
    HoTen: item.HoTen || item.hoTen || item.fullName || '',
    NgaySinh: item.NgaySinh || item.ngaySinh || '',
    GioiTinh: item.GioiTinh || item.gioiTinh || '',
    SucKhoe: item.SucKhoe || item.sucKhoe || 'Ổn định',
...
export default function CreateAdoptionProfile() {
  ...
  const [availableChildren, setAvailableChildren] = useState(fallbackChildren);
  ...
  useEffect(() => {
    ...
    childApi
      .getAll({ status: 'Chờ nhận nuôi', page: 1, limit: 200 })
      .then((res) => {
        const items = res?.items ?? res ?? [];
        if (Array.isArray(items) && items.length > 0)
          setAvailableChildren(items.map(normalizeChild));
      })
      .catch(() => {});
  }, [requestId]);

  const eligibleChildren = useMemo(() => {
    const adopterBirthKnown = Boolean(request.NgaySinhNguoiNhan);
    return availableChildren.filter((child) => {
      if (child.TrangThai !== 'Chờ nhận nuôi') return false;
      if (!adopterBirthKnown) return true;
      return isAtLeast20YearsOlder(request.NgaySinhNguoiNhan, child.NgaySinh);
    });
  }, [availableChildren, request]);
  ...
  async function handleCreateMeeting(e) {
    ...
    const thoiGian = `${meetingForm.meetingDate}T${meetingForm.meetingTime}:00`;
    try {
      const created = await meetingApi.create({
        ...
        GhiChuCanBo: meetingForm.note,
      });
      setMeeting({
        MaLichGap: created?.MaLichGap || created?.maLichGap || `LHGM-${Date.now()}`,
        MaYeuCauNhan: request.MaYeuCauNhan,
        MaTre: selectedChild.MaTre,
        ...
        _id: created?.MaLichGap || created?.maLichGap,
      });
    } catch {
      setMeeting({
        MaLichGap: `LHGM-${Date.now()}`,
        MaYeuCauNhan: request.MaYeuCauNhan,
        MaTre: selectedChild.MaTre,
        TenTre: selectedChild.HoTen,
        TrangThai: 'Chờ xác nhận',
  ...
  async function handleConfirmMeeting() {
    if (meeting?._id) {
      try { await meetingApi.confirm(meeting._id); } catch { /* non-blocking */ }
    }
    setMeeting((prev) => ({ ...prev, TrangThai: 'Đã xác nhận' }));
  }
  ...
  async function handleSubmitProfile() {
    ...
    if (meeting?._id && meetingResult.result) {
      try {
        await meetingApi.recordResult(meeting._id, {
          KetQua: meetingResult.result,
          NgayGapMat: new Date().toISOString(),
          GhiChu: meetingResult.note,
        });
      } catch { /* non-blocking */ }
    }
